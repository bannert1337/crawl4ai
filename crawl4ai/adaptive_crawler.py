...
class CrawlState:
    ...
    document_frequencies: Dict[str, int] = field(
        default_factory=lambda: defaultdict(int)
    )
    documents_with_terms: Dict[str, Set[int]] = field(
    ...
    semantic_gaps: List[Tuple[List[float], float]] = field(
    ...
    def save(self, path: Union[str, Path]):
        ...
        state_dict = {
            ...
            "knowledge_base": [
                self._crawl_result_to_dict(cr) for cr in self.knowledge_base
            ],
            ...
            "documents_with_terms": {
                k: list(v) for k, v in self.documents_with_terms.items()
            },
            ...
            "kb_embeddings": self.kb_embeddings.tolist()
            if self.kb_embeddings is not None
            else None,
            "query_embeddings": self.query_embeddings.tolist()
    ...
    def load(cls, path: Union[str, Path]) -> "CrawlState":
        ...
        state.knowledge_base = [
            cls._dict_to_crawl_result(d) for d in state_dict["knowledge_base"]
        ]
        state.pending_links = [
            Link(**link_dict) for link_dict in state_dict["pending_links"]
        ]
        ...
        state.document_frequencies = defaultdict(
            int, state_dict["document_frequencies"]
        )
        state.documents_with_terms = defaultdict(
            set, {k: set(v) for k, v in state_dict["documents_with_terms"].items()}
        )
        ...
        state.kb_embeddings = (
            np.array(state_dict["kb_embeddings"])
            if state_dict.get("kb_embeddings") is not None
            else None
        )
        state.query_embeddings = (
            np.array(state_dict["query_embeddings"])
            if state_dict.get("query_embeddings") is not None
            else None
        )
...
class AdaptiveConfig:
    ...
    embedding_llm_config: Optional[Union[LLMConfig, Dict]] = (
        None  # Separate config for embeddings
    )
    query_llm_config: Optional[Union[LLMConfig, Dict]] = (
        None  # Config for query expansion (chat completion)
    )
    ...
    embedding_min_confidence_threshold: float = (
        0.1  # Below this, content is considered completely irrelevant
    )
    ...
    embedding_coverage_radius: float = (
        0.2  # Distance threshold for "covered" query points
    )
    ...
    embedding_k_exp: float = (
        1.0  # Exponential decay factor for distance-to-score mapping
    )
    ...
    embedding_nearest_weight: float = (
        0.7  # Weight for nearest neighbor in hybrid scoring
    )
    ...
    embedding_overlap_threshold: float = (
        0.85  # Similarity threshold for penalizing redundant links
    )
    ...
    embedding_min_relative_improvement: float = (
        0.1  # Minimum relative improvement to continue
    )
    ...
    embedding_validation_min_score: float = (
        0.3  # Minimum validation score to trust convergence
    )
    ...
    def validate(self):
        ...
        assert 0 <= self.min_gain_threshold <= 1, (
            "min_gain_threshold must be between 0 and 1"
        )

        # Check weights sum to 1
        weight_sum = (
            self.coverage_weight + self.consistency_weight + self.saturation_weight
        )
        assert abs(weight_sum - 1.0) < 0.001, (
            f"Coverage weights must sum to 1, got {weight_sum}"
        )

        weight_sum = self.relevance_weight + self.novelty_weight + self.authority_weight
        assert abs(weight_sum - 1.0) < 0.001, (
            f"Link scoring weights must sum to 1, got {weight_sum}"
        )

        # Validate embedding parameters
        assert 0 < self.embedding_coverage_radius < 1, (
            "embedding_coverage_radius must be between 0 and 1"
        )
        assert self.embedding_k_exp > 0, "embedding_k_exp must be positive"
        assert 0 <= self.embedding_nearest_weight <= 1, (
            "embedding_nearest_weight must be between 0 and 1"
        )
        assert 0 <= self.embedding_top_k_weight <= 1, (
            "embedding_top_k_weight must be between 0 and 1"
        )
        assert (
            abs(self.embedding_nearest_weight + self.embedding_top_k_weight - 1.0)
            < 0.001
        ), "Embedding weights must sum to 1"
        assert 0 <= self.embedding_overlap_threshold <= 1, (
            "embedding_overlap_threshold must be between 0 and 1"
        )
        assert 0 < self.embedding_min_relative_improvement < 1, (
            "embedding_min_relative_improvement must be between 0 and 1"
        )
        assert 0 <= self.embedding_validation_min_score <= 1, (
            "embedding_validation_min_score must be between 0 and 1"
        )
        assert 0 <= self.embedding_quality_min_confidence <= 1, (
            "embedding_quality_min_confidence must be between 0 and 1"
        )
        assert 0 <= self.embedding_quality_max_confidence <= 1, (
            "embedding_quality_max_confidence must be between 0 and 1"
        )
        assert self.embedding_quality_scale_factor > 0, (
            "embedding_quality_scale_factor must be positive"
        )
        assert 0 <= self.embedding_min_confidence_threshold <= 1, (
            "embedding_min_confidence_threshold must be between 0 and 1"
        )
...
class CrawlStrategy(ABC):
    ...
    async def rank_links(
        self, state: CrawlState, config: AdaptiveConfig
    ...
    async def update_state(
        self, state: CrawlState, new_results: List[CrawlResult]
    ) -> None:
...
class StatisticalStrategy(CrawlStrategy):
    ...
    def _calculate_coverage(self, state: CrawlState) -> float:
        ...
        for term in query_terms:
            ...
            if df > 0:
                ...
                freq_signal = (
                    math.log(1 + tf) / math.log(1 + max_tf) if max_tf > 0 else 0
                )
    ...
    def _calculate_saturation(self, state: CrawlState) -> float:
        ...
        recent_rate = (
            state.new_terms_history[-1] if state.new_terms_history[-1] > 0 else 1
        )
        initial_rate = (
            state.new_terms_history[0] if state.new_terms_history[0] > 0 else 1
        )
        ...
        link_text = " ".join(
            filter(
                ...
                [
                    link.text or "",
                    link.title or "",
                    link.head_data.get("meta", {}).get("title", "")
                    if link.head_data
                    else "",
                    link.head_data.get("meta", {}).get("description", "")
                    if link.head_data
                    else "",
                    link.head_data.get("meta", {}).get("keywords", "")
                    if link.head_data
                    else "",
                ],
            )
...
class EmbeddingStrategy(CrawlStrategy):
    ...
    def __init__(
        ...
        query_llm_config: Union[LLMConfig, Dict] = None,
    ):
    ...
    async def _get_embeddings(self, texts: List[str]) -> Any:
        ...
        return await get_text_embeddings(
            texts, embedding_llm_config, self.embedding_model
        )

    def _compute_distance_matrix(
        self, query_embeddings: Any, kb_embeddings: Any
    ) -> Any:
    ...
    def _get_cached_distance_matrix(
        self, query_embeddings: Any, kb_embeddings: Any
    ) -> Any:
        ...
        if self._distance_matrix_cache is None or kb_hash != self._kb_embeddings_hash:
            # Recompute matrix
            self._distance_matrix_cache = self._compute_distance_matrix(
                query_embeddings, kb_embeddings
            )
    ...
    async def map_query_semantic_space(self, query: str, n_synthetic: int = 10) -> Any:
        ...
        provider = (
            llm_config_dict.get("provider", "openai/gpt-4o-mini")
            if llm_config_dict
        ...
        response = perform_completion_with_backoff(
            ...
            exponential_factor=llm_config_dict.get("backoff_exponential_factor", 2)
            if llm_config_dict
            else 2,
        )
        ...
        n_validation = max(
            2, int(len(other_queries) * 0.2)
        )  # At least 2 for validation
    ...
    def compute_coverage_shape(self, query_points: Any, alpha: float = 0.5):
        ...
        try:
            ...
            coverage = {
                ...
                "points": query_points,
                "radius": np.max(
                    np.linalg.norm(query_points - np.mean(query_points, axis=0), axis=1)
                ),
            }
            return coverage
        except Exception:
    ...
    def find_coverage_gaps(
        self, kb_embeddings: Any, query_embeddings: Any
        ...
        distance_matrix = self._get_cached_distance_matrix(
            query_embeddings, kb_embeddings
        )
    ...
    async def select_links_for_expansion(
            ...
            link_text = " ".join(
                filter(
                    ...
                    [
                        link.text or "",
                        link.title or "",
                        link.meta.get("description", "")
                        if hasattr(link, "meta") and link.meta
                        else "",
                        link.head_data.get("meta", {}).get("description", "")
                        if link.head_data
                        else "",
        ...
        if texts_to_embed:
            embedding_llm_config = self._get_embedding_llm_config_dict()
            new_embeddings = await get_text_embeddings(
                texts_to_embed, embedding_llm_config, self.embedding_model
            )

            # Cache the new embeddings
            for link, text, embedding in zip(
                links_to_embed, texts_to_embed, new_embeddings
            ):
        ...
        coverage_radius = (
            self.config.embedding_coverage_radius if hasattr(self, "config") else 0.2
        )
        ...
        for link in candidate_links:
            ...
            if not gaps:
                ...
                if kb_embeddings is not None and len(kb_embeddings) > 0:
                    ...
                    kb_norm = kb_embeddings / np.linalg.norm(
                    ...
                    overlap_threshold = (
                        self.config.embedding_overlap_threshold
                        if hasattr(self, "config")
                        else 0.85
                    )
                    if max_similarity > overlap_threshold:
                        overlap_penalty = (
                            max_similarity - overlap_threshold
                        ) * 2  # 0 to 0.3 range
                    else:
                        overlap_penalty = 0
                else:
    ...
    async def rank_links(
        ...
        gaps = self.find_coverage_gaps(state.kb_embeddings, state.query_embeddings)
        state.semantic_gaps = [
            (g[0].tolist(), g[1]) for g in gaps
    ...
    async def validate_coverage(self, state: CrawlState) -> float:
        ...
        if self._validation_embeddings_cache is None:
            self._validation_embeddings_cache = await self._get_embeddings(
        ...
        distance_matrix = self._compute_distance_matrix(
            val_embeddings, state.kb_embeddings
        )
    ...
    async def should_stop(self, state: CrawlState, config: AdaptiveConfig) -> bool:
        ...
        min_confidence_threshold = (
            config.embedding_min_confidence_threshold
            if hasattr(config, "embedding_min_confidence_threshold")
            else 0.1
        )
        ...
        improvement_diffs = list(
            zip(state.confidence_history[:-1], state.confidence_history[1:])
        )

        # Calculate average improvement
        avg_improvement = sum(abs(b - a) for a, b in improvement_diffs) / len(
            improvement_diffs
        )
        state.metrics["avg_improvement"] = avg_improvement

        min_relative_improvement = (
            self.config.embedding_min_relative_improvement * confidence
            if hasattr(self, "config")
            else 0.1 * confidence
        )
        if avg_improvement < min_relative_improvement:
            ...
            validation_min = (
                self.config.embedding_validation_min_score
                if hasattr(self, "config")
                else 0.4
            )
            # k_exp = self.config.embedding_k_exp if hasattr(self, 'config') else 1.0
    ...
    def get_quality_confidence(self, state: CrawlState) -> float:
        ...
        scale_factor = (
            self.config.embedding_quality_scale_factor
            if hasattr(self, "config")
            else 0.833
        )
    ...
    async def update_state(
        ...
        embedding_llm_config = self._get_embedding_llm_config_dict()
        new_embeddings = await get_text_embeddings(
            new_texts, embedding_llm_config, self.embedding_model
        )
        ...
        if state.kb_embeddings is None:
            ...
            for i, new_emb in enumerate(new_embeddings):
                ...
                kb_normalized = state.kb_embeddings / np.linalg.norm(
                    state.kb_embeddings, axis=1, keepdims=True
                )
            ...
            if deduplicated_embeddings:
                state.kb_embeddings = np.vstack(
                    [state.kb_embeddings, np.array(deduplicated_embeddings)]
                )
        ...
        if hasattr(state, "query_embeddings") and state.query_embeddings is not None:
            state.coverage_shape = self.compute_coverage_shape(
                state.query_embeddings,
                self.config.alpha_shape_alpha if hasattr(self, "config") else 0.5,
            )
...
class AdaptiveCrawler:
    ...
    async def digest(
        self, start_url: str, query: str, resume_from: Optional[str] = None
    ) -> CrawlState:
        ...
        if isinstance(self.strategy, EmbeddingStrategy) and not resume_from:
            ...
            (
                query_embeddings,
                expanded_queries,
            ) = await self.strategy.map_query_semantic_space(
                query, self.config.n_query_variations
            )
        ...
        try:
                ...
                if result and hasattr(result, "success") and result.success:
                    ...
                    if hasattr(result, "links") and result.links:
                        if isinstance(result.links, dict):
                            ...
                            self.state.pending_links.extend(
                    ...
                    if base_domain:
                        self.state.pending_links = [
                            link
                            for link in self.state.pending_links
                            if not link.href
                            or not is_external_url(link.href, base_domain)
                        ]
            ...
            while depth < self.config.max_depth:
                ...
                to_crawl = [
                    (link, score)
                    for link, score in ranked_links[: self.config.top_k_links]
                        ...
                        if result:
                            ...
                            if hasattr(result, "links") and result.links:
                                ...
                                if isinstance(result.links, dict):
                                    ...
                                    internal_links = [
                                        Link(**link_data)
                                        for link_data in result.links.get(
                                            "internal", []
                                        )
                                    ]
                                    external_links = [
            ...
            if isinstance(self.strategy, EmbeddingStrategy):
                self.state.metrics["confidence"] = self.strategy.get_quality_confidence(
                    self.state
                )
            else:
    ...
    async def _crawl_batch(
        self, links_with_scores: List[Tuple[Link, float]], query: str
    ) -> List[CrawlResult]:
        ...
        for result in results:
            if isinstance(result, CrawlResult):
                ...
                if hasattr(result, "success") and result.success:
                    ...
                    print(
                        f"Skipping failed crawl: {result.url if hasattr(result, 'url') else 'unknown'}"
                    )
    ...
    def print_stats(self, detailed: bool = False) -> None:
        ...
        if not detailed and use_rich:
            ...
            table.add_row(
                "Content Length", f"{stats.get('total_content_length', 0):,} chars"
            )
            ...
            if isinstance(self.strategy, EmbeddingStrategy):
                ...
                table.add_row(
                    "Avg Min Distance",
                    f"{self.state.metrics.get('avg_min_distance', 0):.3f}",
                )
                table.add_row(
                    "Avg Close Neighbors",
                    f"{self.state.metrics.get('avg_close_neighbors', 0):.1f}",
                )
                table.add_row(
                    "Validation Score",
                    f"{self.state.metrics.get('validation_confidence', 0):.2%}",
                )
                table.add_row("", "")  # Spacer
                table.add_row(
                    "Is Sufficient?",
                    "[green]Yes (Validated)[/green]"
                ...
                table.add_row(
                    "Is Sufficient?",
                    "[green]Yes[/green]" if self.is_sufficient else "[red]No[/red]",
                )
            ...
            if isinstance(self.strategy, EmbeddingStrategy):
                ...
                print(
                    f"  Average Min Distance: {self.state.metrics.get('avg_min_distance', 0):.3f}"
                )
                print(
                    f"  Avg Close Neighbors (< 0.3): {self.state.metrics.get('avg_close_neighbors', 0):.1f}"
                )
                print(
                    f"  Avg Very Close Neighbors (< 0.2): {self.state.metrics.get('avg_very_close_neighbors', 0):.1f}"
                )
                ...
                if self.is_sufficient:
                    if use_rich:
                        console.print(
                            f"  Overall Confidence: {self.confidence:.2%} [green][VALIDATED][/green]"
                        )
                    else:
                        print(
                            f"  Overall Confidence: {self.confidence:.2%} [VALIDATED]"
                        )
                else:
                    if use_rich:
                        console.print(
                            f"  Overall Confidence: {self.confidence:.2%} [red][NOT VALIDATED][/red]"
                        )
                    else:
                        print(
                            f"  Overall Confidence: {self.confidence:.2%} [NOT VALIDATED]"
                        )

                print(
                    f"  Learning Score: {self.state.metrics.get('learning_score', 0):.2%}"
                )
                print(
                    f"  Validation Score: {self.state.metrics.get('validation_confidence', 0):.2%}"
                )
                ...
                for term in query_terms:
                    ...
                    if df > 0:
                        if use_rich:
                            console.print(
                                f"  '{term}': found in {df}/{self.state.total_documents} docs ([green]{df / self.state.total_documents:.0%}[/green]), {tf} occurrences"
                            )
                        else:
                            print(
                                f"  '{term}': found in {df}/{self.state.total_documents} docs ({df / self.state.total_documents:.0%}), {tf} occurrences"
                            )
                    else:
                        if use_rich:
                ...
                if use_rich:
                    ...
                    console.print(
                        f"  Overall Confidence: {self.confidence:.2%} {status_colored}"
                    )
                else:
                    print(f"  Overall Confidence: {self.confidence:.2%} {status}")
                print(f"  Coverage Score: {self.state.metrics.get('coverage', 0):.2%}")
                print(
                    f"  Consistency Score: {self.state.metrics.get('consistency', 0):.2%}"
                )
                print(
                    f"  Saturation Score: {self.state.metrics.get('saturation', 0):.2%}"
                )

            # Crawl efficiency
            if self.state.new_terms_history:
                avg_new_terms = sum(self.state.new_terms_history) / len(
                    self.state.new_terms_history
                )
                print(f"\n[*] Crawl Efficiency:")
                print(f"  Avg New Terms per Page: {avg_new_terms:.1f}")
                print(
                    f"  Information Saturation: {self.state.metrics.get('saturation', 0):.2%}"
                )
            ...
            if detailed:
                ...
                for i, (term, freq) in enumerate(top_terms, 1):
                    ...
                    if use_rich:
                        console.print(
                            f"  {i:2d}. [yellow]'{term}'[/yellow]: {freq} occurrences in {df} docs"
                        )
                    else:
                ...
                for i, url in enumerate(self.state.crawl_order, 1):
                    new_terms = (
                        self.state.new_terms_history[i - 1]
                        if i <= len(self.state.new_terms_history)
                        else 0
                    )
                    if use_rich:
                        console.print(f"  {i}. [cyan]{url}[/cyan]")
                        console.print(
                            f"     -> Added [green]{new_terms}[/green] new terms"
                        )
                    else:
                        print(f"  {i}. {url}")
                ...
                if self.state.embedding_model:
                    ...
                    if self.state.kb_embeddings is not None:
                        print(
                            f"  Knowledge Embeddings: {self.state.kb_embeddings.shape}"
                        )
                    else:
    ...
    def export_knowledge_base(
        self, filepath: Union[str, Path], format: str = "jsonl"
    ) -> None:
    ...
    def _crawl_result_to_export_dict(self, result) -> Dict[str, Any]:
        ...
        if self.state:
            export_dict["crawl_metadata"] = {
                "crawl_order": self.state.crawl_order.index(export_dict["url"]) + 1
                if export_dict["url"] in self.state.crawl_order
                else 0,
